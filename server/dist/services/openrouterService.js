...
function getProfileFromRequest(req) {
    ...
    if (req.profileCode) {
        ...
        return {
            ...
            visual: (parts[1] === 'P' ? 'P' : 'T'),
            processing: (parts[2] === 'A' ? 'A' : 'G'),
            tempo: (parts[3] === 'R' ? 'R' : 'I')
        };
    }
    // Default profile
...
function getWordLimitByProfile(tempo) {
    return tempo === 'I'
        ? { min: 30, max: 100, target: 60 }
        : { min: 120, max: 300, target: 200 };
}
// ===== MODALITY INSTRUCTION =====
function getModalityInstruction(modality) {
    if (modality === 'T') {
        return `
MODALITY: TEXT/LOGIC
...
function getStructureInstruction(structure) {
    if (structure === 'G') {
        return `
...
function getPersonalizedMotivation(profile) {
...
function buildPersonalizedSystemPrompt(profile) {
    ...
    const strategy = (0, profileSystem_1.getAIStrategy)(`${profile.level}${profile.visual}${profile.processing}${profile.tempo}`);
...
async function generateAdaptiveFeedback(req) {
...
`;
    try {
        ...
        if (wordCount < wordLimit.min || wordCount > wordLimit.max) {
            console.warn(`⚠️ Word count ${wordCount} violates limits [${wordLimit.min}-${wordLimit.max}]`);
        }
        return {
            feedback,
            wordCount,
...
async function generateCorrectiveFeedbackForResearch(req) {
    const profileParts = req.profileCode.toUpperCase();
    const profile = {
        level: parseInt(profileParts[0]) || 3,
        visual: (profileParts[1] === 'P' ? 'P' : 'T'),
        processing: (profileParts[2] === 'A' ? 'A' : 'G'),
        tempo: (profileParts[3] === 'R' ? 'R' : 'I')
    };
