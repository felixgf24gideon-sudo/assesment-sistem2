...
exports.parseProfileCode = parseProfileCode;
exports.deriveAIStrategy = deriveAIStrategy;
exports.getAIStrategy = getAIStrategy;
exports.getProfileDescription = getProfileDescription;
exports.getAllProfileCodes = getAllProfileCodes;
exports.isValidProfileCode = isValidProfileCode;
function parseProfileCode(code) {
    const cleanCode = (code || '3TGI').toUpperCase().trim();
    return {
        level: parseInt(cleanCode[0]) || 3,
        visual: (cleanCode[1] === 'P' ? 'P' : 'T'),
        processing: (cleanCode[2] === 'A' ? 'A' : 'G'),
        tempo: (cleanCode[3] === 'R' ? 'R' : 'I')
    };
}
 ...
 */
function generateOpeningPhrase(level, visual, processing, tempo) {
 ...
 */
function generateClosingStyle(level, visual, processing, tempo) {
    const supportLevel = level <= 2 ? 'very_high' : level <= 3 ? 'high' : 'moderate';
...
function deriveAIStrategy(params) {
    ...
    if (level >= 5) {
        hintProgression = hintProgression.map(hint => hint.replace('Sebutkan', 'Consider')
...
function getAIStrategy(profileCode) {
...
function isValidProfileCode(code) {
