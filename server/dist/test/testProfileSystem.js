...
testCodes.forEach(code => {
    const isValid = (0, profileSystem_1.isValidProfileCode)(code);
    console.log(`${code}: ${isValid ? '✅ Valid' : '❌ Invalid'}`);
    if (isValid) {
        const params = (0, profileSystem_1.parseProfileCode)(code);
        console.log(`   Level: ${params.level}, Visual: ${params.visual}, Processing: ${params.processing}, Tempo: ${params.tempo}`);
        console.log(`   Description: ${(0, profileSystem_1.getProfileDescription)(code)}`);
    }
    console.log('');
});
...
sampleProfiles.forEach(code => {
    console.log(`\n Profile: ${code}`);
    console.log(`   ${(0, profileSystem_1.getProfileDescription)(code)}`);
    const strategy = (0, profileSystem_1.getAIStrategy)(code);
    console.log('\n   Corrective Feedback:');
...
console.log('-'.repeat(60));
const allCodes = (0, profileSystem_1.getAllProfileCodes)();
console.log(`Total profiles: ${allCodes.length} (expected: 48)`);
console.log(`All valid: ${allCodes.every(code => (0, profileSystem_1.isValidProfileCode)(code)) ? '✅ YES' : '❌ NO'}`);
...
const impulsiveLimits = impulsiveProfiles.map(c => (0, profileSystem_1.getAIStrategy)(c).correctiveFeedback.maxWords);
const reflectiveLimits = reflectiveProfiles.map(c => (0, profileSystem_1.getAIStrategy)(c).correctiveFeedback.maxWords);
