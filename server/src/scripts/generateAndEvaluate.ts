...
function getTempo(profileCode: string): 'I' | 'R' {
  return profileCode[3] as 'I' | 'R';
}
...
function calculateMetrics(dataset: DatasetEntry[]) {
...
function displayMetrics(metrics: any): void {
  console.log('\n' + '═'.repeat(70));
...
async function generatePersonalizedDataset(useTestMode: boolean = true): Promise<void> {
  ...
  for (const profile of profiles) {
    ...
    for (const question of questions) {
      ...
      try {
        ...
        const evaluation = await evaluateFeedback({
          profile: profile,
