...
interface RefinedEvaluationResult {
  // 6 Dimensi Utama
  instructional_quality: number;
  instructional_reasoning: string;

  clarity_and_practicality: number;
  clarity_reasoning: string;

  modality_alignment: number;
  modality_reasoning: string;

  structure_alignment: number;
  structure_reasoning: string;

  tempo_alignment: number;
  tempo_reasoning: string;

  personalized_motivation: number;
  motivation_reasoning: string;

  // Computed scores
  overall_personalization_score: number;
...
function parseProfile(profileCode: string) {
  return {
...
function buildRefinedContext(profileCode: string, wordCount: number): string {
...
function buildRefinedEvaluationPrompt(req: EvaluationRequest): string {
...
function validateScore(value: any, defaultValue: number = 3): number {
...
export async function evaluateFeedback(
...
): Promise<RefinedEvaluationResult> {
  ...
  try {
    ...
    const overallPersonalizationScore =
...
// ✅ EXPORTS
export { RefinedEvaluationResult, EvaluationRequest };