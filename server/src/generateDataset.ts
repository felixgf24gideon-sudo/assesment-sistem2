  ...
  for (const profile of ALL_PROFILES) {
    ...
    for (const question of questions) {
      ...
      try {
        ...
        const feedback1 = await generateCorrectiveFeedback({
          profileCode: profile,
      ...
      try {
        ...
        const feedback2 = await generateCorrectiveFeedback({
          profileCode: profile,
          questionText: question.text,
          correctAnswer: correctAnswerText,
          studentAnswer: wrongAnswerText,
...
function saveDataset(dataset: DatasetEntry[], suffix: string) {
...
function generateSummary(dataset: DatasetEntry[]) {
  console.log('\n📊 DATASET SUMMARY:');
  console.log('═'.repeat(60));
